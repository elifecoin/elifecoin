...
// Copyright (c) 2017 The ElifeCoin developers
...
#if defined(NDEBUG)
# error "ElifeCoin cannot be compiled without assertions."
#endif
...
CBigNum bnProofOfStakeLimit(~uint256(0) >> 54);
CBigNum bnProofOfStakeLimitV2(~uint256(0) >> 50);
...
{
    if (nHeight+1000 > Params().LastPOWBlock())
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight)
{
    ...
    int64_t nRewardCoinYear, nSubsidy;
    ...
    if(nHeight <= 3 * YEARLY_BLOCKCOUNT)
        ...
        nRewardCoinYear = 2 * COIN_YEAR_REWARD;
    else if(nHeight <= (18 * YEARLY_BLOCKCOUNT))
        nRewardCoinYear = 1 * COIN_YEAR_REWARD;
    else if(nHeight > (18 * YEARLY_BLOCKCOUNT))
        nRewardCoinYear = 0.1 * COIN_YEAR_REWARD;
...
{
    RenameThread("elifecoin-loadblk");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        {
            LogPrintf("net", "connected to self at %s, disconnecting\n", pfrom->addr.ToString());
            pfrom->fDisconnect = true;
            return true;
        }
